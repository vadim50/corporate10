...
<div class="hentry hentry-post blog-big group ">
    ...
    <div class="meta group">
        <p class="author">
        	<span>by <a href="#" title="{{ $article->title }}" rel="author">
        		{{ $article->user->name }}
        	</a></span></p>
        <p class="categories">
        	<span>In: <a href="{{ route('articlesCat',['cat_alias'=>$article->category->alias]) }}" title="View all posts in {{ $article->category->title }}" rel="category tag">
        		{{ $article->category->title }}
        	
        	</a></span></p>
        <p class="comments"><span><a href="#comments" title="Comment on This is the title of the first article. Enjoy it.">
...
<div id="comments">
    <h3 id="comments-title">
        <span>{{ count($article->comments) ? count($article->comments) : '0' }}</span> {{ Lang::choice('ru.comments',$article->comments) }}    
    </h3>
